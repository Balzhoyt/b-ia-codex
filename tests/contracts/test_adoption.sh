...
make_dependency_path "$tmp/bin-full" \
  bash env dirname realpath git codex gentle-ai engram jq mktemp find sed sort awk uniq comm tr \
...
adoption_output="$("$adopt" --root "$root" --consumer "$consumer" --manifest "$manifest")"
printf '%s\n' "$adoption_output"
...
for kind in symlink hardlink; do
  ...
  set +e; "$adopt" --root "$root" --consumer "$alias_consumer" --manifest "$manifest" >/dev/null 2>&1; alias_code=$?; set -e
  test "$alias_code" -eq 10; test "$(cat "$outside")" = outside
done

bad="$tmp/bad.tsv"
printf 'canonical-source\ttemplates/codex/AGENTS.md\t../escape.md\tcopy\n' > "$bad"
set +e; "$adopt" --root "$root" --consumer "$consumer" --manifest "$bad"; code=$?; set -e
test "$code" -eq 10; test ! -e "$tmp/escape.md"

incomplete="$tmp/incomplete.tsv"; grep -v 'templates/codex/AGENTS.md' "$manifest" > "$incomplete"
set +e; "$adopt" --root "$root" --consumer "$consumer" --manifest "$incomplete" --dry-run; incomplete_code=$?; set -e
...
set +e; "$adopt" --root "$root" --consumer "$consumer" --manifest "$extra" --dry-run; extra_code=$?; set -e
...
set +e; "$adopt" --root "$root" --consumer "$consumer" --manifest "$duplicate_destination" --dry-run; duplicate_destination_code=$?; set -e
